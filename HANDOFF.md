...
**Last verified against the live codebase + database: 2026-08-09 (admin RSVPs tab gained a live people-level Headcount panel: max possible / attending / declined / awaiting / open slots / still possible, derived from `computeHeadcount` in `src/lib/rsvp-party.ts`; build + typecheck pass)** Bump this line whenever you re-verify. Read `ONBOARDING.md` first — it's the current-state reference; this file is the narrative behind decisions.
...
- Verified end to end (2026-08-04) against a seeded matrix of **10 `ZZTEST` households** (`ZZT001`–`ZZT010`) covering both verification factors, the implicit `named + 1` fallback, explicit caps with two/one/zero open slots, name-to-come placeholders, submission with and without an email address, a full decline, and a mobile (440px) pass. Confirmed: wrong verification answer rejected then accepted on retry; ZIP-only households get the ZIP prompt; add buttons disappear exactly at the cap; blank pending names land in the DB as `Guest of <household>` with `name_pending: true`; `added_by_guest` is stamped server-side; re-opening a submitted RSVP lets the household fill the pending name in place (one `rsvps` row, updated not duplicated); admin badges render and **Add all guest-added names to the invite list** folds both added names into `party_members`. Emails fired for real: `rsvp-confirmation` sent to the test address, `admin-notification` sent per submission (`email_send_log` = 69 rows). All 10 test households and their RSVPs were purged afterward (`guests` = 86, `ZZTEST` = 0, `rsvps` = 0).
- **Resolved 2026-08-04:** `ADMIN_NOTIFICATION_EMAILS` was `noreply@notify.morenowedding2026.com`, so admin RSVP alerts landed in the sending mailbox. Confirmed by query — every `admin-notification` row in `email_send_log` had that recipient, and the only `geoddison@gmail.com` row was an `rsvp-confirmation` (the guest copy for a ZZTEST household that used a `+zztest8` address). - **The Master CSV importer is intentionally upsert-only and RSVP-safe.** `planImportRows()` resolves every row as insert/update/error before any write happens, and the commit phase only touches the `guests` table. Existing RSVPs are never modified, deleted, or reset. Match precedence is `slug` → phone → email, so including the `slug` column on an update is the safest way to avoid accidentally creating a duplicate household. Blank optional cells on an update are omitted from the payload, which means they leave the existing value in place rather than clearing it. The one exception is `country`: new rows default to "USA" when blank, but update rows leave the stored country alone. This design lets the couple add last-minute households after launch without risking the RSVPs already collected.
...
As of this handoff: the site is live and accepting RSVPs. **The `guests` table holds 78 real households, 0 `ZZTEST` test rows, and 0 RSVPs** (verified by query 2026-08-06, after the placeholder cleanup and the final smoke-test purge). Every household has a verification factor; 40 households have an explicit `max_party_size`, and the remaining 38 use the fallback limit (everyone named, plus one open slot). `rsvp_open` was flipped **on** 2026-08-05; `guest_photo_uploads` and `show_wedding_party` remain off (couple's choice, not a blocker). A final end-to-end smoke test passed 2026-08-05: lookup → phone verification → submit → admin notification delivered to `geoddison@gmail.com`. The master guest list was exported 2026-08-06 to `/mnt/documents/morenowedding2026-master-guest-list-2026-08-05.csv` (78 rows, round-trippable). Remaining optional work: filling in known individual names inside households, reviewing the 38 fallback-limit households for plus-one correctness, wedding-party personalization copy (see §5), and real per-entry Story photos.

**RSVP verification is per-household, and guests no longer touch their address (2026-07-30).** Invitations were about to go out against a guest list where the couple has every mailing address but only some phone numbers, and the old flow hard-required phone last-4. Rather than downgrade everyone to the weakest available factor for uniformity, the server now picks the strongest factor it has per household: phone last-4 when a phone exists, 5-digit mailing ZIP otherwise. Guests are never shown a choice and there is deliberately no "try the other one" fallback — that branching is the only thing that would have made this flow feel complicated. ZIP is the weaker secret (guessable by anyone who knows roughly where a family lives), which is exactly why it's the fallback and not the default; the existing 5-attempt/15-minute lockout and per-IP search rate limit keep it proportionate to the real threat model here (a nosy acquaintance, not an attacker). Verification was *not* removed, because the invitation list is itself the private data: without a check, anyone who guesses a surname could read who else was invited to that household and change their RSVP. TextMyWedding links still only carry the household — they land the guest on their own invitation and then still ask the question, so a forwarded or intercepted text doesn't hand anything over. Two data-model guardrails shipped with it: `guests.phone` is nullable (so address-only households can be imported without inventing a fake number that becomes an unguessable last-4), and a `guests_has_verify_factor` check constraint makes it impossible to create a household nobody can ever verify into. Separately, the mailing-address confirm/edit block and the `updateGuestAddress` server function were deleted: the couple already has the addresses, so re-confirming them added a step, invited typos into a clean dataset, and put a full street address on screen for no benefit — and `PublicGuest` now excludes `slug`, `phone`, and `address` entirely so they never reach the browser or a devtools panel. Addresses remain editable in the admin dashboard, which is now their single home.
...
**Feature flags:** a `feature_flags` table (`key`, `enabled`, `updated_at`) plus `src/lib/feature-flags.functions.ts` (server functions) and `src/hooks/use-feature-flags.ts` (client hook). The admin Features tab uses a **draft → confirm → save** UX (toggle a switch, it stages a change; nothing hits the server until you click Save and confirm a diff of what's about to change) — this was an explicit design ask from the couple, not a default pattern, so preserve it if you touch that panel. Four flags exist today: `rsvp_open`, `guest_photo_uploads`, `show_ushers`, `show_wedding_party`. The last one was added 2026-08-04 to unpublish the Wedding Party section without deleting it: rather than a branch or a code removal, the flag hides the section, its nav link, its spine numeral, and the MCP `get_wedding_party` tool, and `src/hooks/use-section-order.ts` renumbers the remaining sections so the numerals stay contiguous. Turning it on is a one-switch restore. Check the live table before assuming any of their values — see ONBOARDING.md §2 for the last-verified snapshot.
...
- **The day-of schedule was reorganized against Travel, then rewritten twice more as the couple's real timeline firmed up (2026-07-21).** Address/map/directions/parking moved out of the Travel section into a new "Arrival & Parking" block in `DaySection.tsx`'s right column (day-of logistics live next to the schedule, not in a separate section a guest has to go find); `TravelSection.tsx` was renamed from "Getting There" to "Travel" — its nav label had said "Travel" all along, an inconsistency this also fixes — and now only covers hotels and weather/packing. Several confirmed-dead `dictionaries.ts` fields turned up along the way and were removed: `travel.mapTitle`/`addressLabel` (declared, never read), `travel.hotelGroups` (an already-drifted duplicate of the real `HOTELS` array in `wedding-data.ts`), and `details.schedule`/`scheduleTitle` (a duplicate schedule copy, zero consumers).
  - **Current shape (as of the second rewrite): `DAY_SCHEDULE` in `wedding-data.ts` is a flat `ScheduleItem[]` (`{ time, label }[]`), 6 rows** — Ceremony (3:00), Cocktail Hour (3:30), Reception Begins (4:30), Dinner (5:00), Dancing and Celebration (7:00), Celebration Concludes (Midnight). No detailed reception program (toasts/dances/cake/tosses were tried in an intermediate 16-row/3-group version and then dropped in favor of this shorter list once the couple finalized their real plan). `DaySection.tsx` renders it as one flat list — no group sub-headers — and `get_schedule` (`src/lib/mcp/tools/schedule.ts`) does a plain `.map`, not a group-flattening one. If you see references to `ScheduleGroup` or a `group` field anywhere, they're stale — that type was introduced and then removed in the same day.
...
2. **Re-import the real guest list.** `guests` is empty (0 rows, verified 2026-07-24); a prior 52-household import is not present in the current environment. Use the admin dashboard's CSV importer. Blocks Sprint 2 from actually being live regardless of the flag state.
...
| 2 — RSVP Launch Readiness | ⚠️ Code done, `rsvp_open` is on, but `guests` is empty — re-import required |
...
- Apply `<picture>`/WebP srcset to the remaining Story section photos once the real per-entry photos land (entry 05 has real proposal photos as of 2026-07-28; every other entry is still placeholder).
