...
As of this handoff: the site is feature-complete for launch. **The `guests` table holds 4 `ZZTEST`-prefixed test households seeded on 2026-08-01 to exercise RSVP verification and submission — 1 RSVP has been submitted (attending).** A prior snapshot showed 52 imported households, but they are not in the current environment; RSVP lookup will fail for every real guest until the household CSV is re-imported via the admin dashboard. `rsvp_open` is on; `guest_photo_uploads` is off (couple's choice, not a blocker). The other outstanding creative task is the wedding-party personalization copy (see §5).
