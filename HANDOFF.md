...
- **Resolved 2026-08-04:** `ADMIN_NOTIFICATION_EMAILS` was `noreply@notify.morenowedding2026.com`, so admin RSVP alerts landed in the sending mailbox. Confirmed by query — every `admin-notification` row in `email_send_log` had that recipient, and the only `geoddison@gmail.com` row was an `rsvp-confirmation` (the guest copy for a ZZTEST household that used a `+zztest8` address). The secret is now `geoddison@gmail.com`. Guest-facing From address unchanged.
