...
As of this handoff: the site is feature-complete for launch. **The `guests` table holds 86 real households, 0 `ZZTEST` test rows, and 0 RSVPs** (verified by query 2026-08-04, after the test matrix was purged). Every household has a verification factor; none has an explicit `max_party_size` yet, so all currently use the fallback limit (everyone named, plus one open slot). `rsvp_open` is on; `guest_photo_uploads` is off (couple's choice, not a blocker). Remaining pre-launch work: filling in known individual names inside households, setting `max_party_size` where the real headcount differs from the default, and the wedding-party personalization copy (see §5).
