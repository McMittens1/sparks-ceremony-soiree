...
**Feature flags:** a `feature_flags` table (`key`, `enabled`, `updated_at`) plus `src/lib/feature-flags.functions.ts` (server functions) and `src/hooks/use-feature-flags.ts` (client hook). The admin Features tab uses a **draft → confirm → save** UX (toggle a switch, it stages a change; nothing hits the server until you click Save and confirm a diff of what's about to change) — this was an explicit design ask from the couple, not a default pattern, so preserve it if you touch that panel. Five flags exist today: `rsvp_open`, `guest_photo_uploads`, `show_ushers`, `show_wedding_party`, `show_portraits`. The last was added 2026-08-10 alongside the Portraits gallery; it ships **on** because that section uses real photos, but it follows the same hide-section/renumber mechanism so it can be pulled without a deploy. The last one was added 2026-08-04 to unpublish the Wedding Party section without deleting it: rather than a branch or a code removal, the flag hides the section, its nav link, its spine numeral, and the MCP `get_wedding_party` tool, and `src/hooks/use-section-order.ts` renumbers the remaining sections so the numerals stay contiguous. Turning it on is a one-switch restore. Check the live table before assuming any of their values — see ONBOARDING.md §2 for the last-verified snapshot.
