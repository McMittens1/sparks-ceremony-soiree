...
**Last verified against the live codebase + database: 2026-08-06 (instructional placeholders removed from `party_members`; explicit caps set on 37 households).** Update this line every time you re-verify. If it's stale by more than a session or two, re-verify before trusting any specific claim below — this doc is only useful if it mirrors reality.
...
6. **Don't use the `ZZTEST` prefix for real guests.** `ZZTEST` is reserved for purge-able test data. Real last-minute additions should use normal household names.
7. **Dry-run, then smoke-test.** Always click **Preview** first and review the insert/update/error breakdown. After the real import, add one test household, verify it publicly, submit a test RSVP, then delete the test RSVP to confirm the pipeline still works with real data in the table.
