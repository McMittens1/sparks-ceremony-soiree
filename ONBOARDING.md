...
**Last verified against the live codebase + database: 2026-08-09 (admin RSVPs tab gained a live people-level Headcount panel: max possible / attending / declined / awaiting / open slots / still possible, derived from `computeHeadcount` in `src/lib/rsvp-party.ts`; build + typecheck pass)** Update this line every time you re-verify. If it's stale by more than a session or two, re-verify before trusting any specific claim below — this doc is only useful if it mirrors reality.
