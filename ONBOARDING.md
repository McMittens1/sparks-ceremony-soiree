...
**Last verified against the live codebase + database: 2026-08-06 (importer upgraded: pre-import snapshots with one-click restore, field-level dry-run diffs, RSVP-collision warnings, admin import summary email, dashboard readiness counters; build + typecheck pass).** Update this line every time you re-verify. If it's stale by more than a session or two, re-verify before trusting any specific claim below — this doc is only useful if it mirrors reality.
