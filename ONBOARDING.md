...
- **All homepage photos are served as responsive WebP.** Every `src/assets/**/*.asset.json` original has 480/800/1200/1600px WebP derivatives in `public/images/derived/`, indexed by original filename in `src/lib/image-manifest.json`. `src/components/site/ResponsiveImg.tsx` looks up the manifest by the filename in the asset URL and emits `srcSet` + `sizes` (unknown sources fall through to a plain `<img>`). Used by `StoryTimeline.tsx`, `PortraitsSection.tsx`, and `Lightbox.tsx`. Regenerate derivatives with the PIL script pattern in `HANDOFF.md` after adding assets. Homepage image weight measured live: ~259 KB at 440px, ~424 KB at 1280px (was multiple MB).
...
**Guest data (live, 2026-09-02, re-verified this session by direct query):** `guests` holds **157 households**, **0** `ZZTEST` test rows, and **0** households that can't verify. `max_party_size` is now set on **every** household — no row uses the fallback limit. `rsvps` has **33** responses (**32 attending, 1 declined**), leaving **124 households pending**; **41** of those pending households have a phone number (textable) and **83** are address-only. Headcount: **69 confirmed attending of 483 possible**. Only **14** households have an email on file (email is optional and only controls guest confirmation copies), so an email-only reminder campaign is not viable — the chase workflow in §5 is phone/paper-first. `guest_photos` is empty and the `guest-photos` bucket holds 0 objects. `email_send_log` has **179** rows; every send in the last two weeks resolved `pending` → `sent` (the `failed`/`dlq` rows are all mid-July, before the queue fix). `ADMIN_NOTIFICATION_EMAILS` is `geoddison@gmail.com` and is live on the current deploy.

**Historic snapshot (2026-08-06):** `guests` held **78 real households**, **0** `ZZTEST` test rows, and **0** households that can't verify (every row has a phone or a postal code). `max_party_size` is set on **40** households; the remaining **38** still use the fallback limit (everyone named, plus one open slot). `rsvps` has **0** responses after the final smoke-test RSVP was purged. `guest_photos` is empty and the `guest-photos` storage bucket holds 0 objects. `email_send_log` has 70 rows (audit trail, including the smoke-test admin notification to `geoddison@gmail.com`); `email_send_state` has 1 row; `suppressed_emails` has 2 rows; `analytics_events` has 5 rows. A final end-to-end smoke test was run 2026-08-05 via Playwright (lookup → phone verification → submit), the RSVP row was written, an `admin-notification` email was logged as sent to `geoddison@gmail.com`, and the test household was purged. **Master CSV exported** on 2026-08-06 to `/mnt/documents/morenowedding2026-master-guest-list-2026-08-05.csv` (78 rows, 28 columns, round-trippable). `ADMIN_NOTIFICATION_EMAILS` is `geoddison@gmail.com` and is live on the current deploy.
...
**Image optimization (live, 2026-09-02):** Hero portrait and venue aerial use hand-made WebP variants in `public/images/`; every other photo (Story clusters, Portraits grid, Lightbox) goes through `ResponsiveImg` + `public/images/derived/`. All non-hero photos are `loading="lazy"` / `decoding="async"`; the first three Portraits thumbs stay eager. No console errors on the homepage at 440px or 1280px.
...
- **RSVPs tab — chasing non-responders:** a **"Still to chase"** panel sits between the filter toolbar and the table whenever any real household has no RSVP. It shows the total non-responders, how many have a phone (textable) vs are address-only, and days remaining until `SITE.rsvpDeadline`. Those counts come from every real household, not the filtered view, so they stay stable while you narrow the table. **Text list** and **Mail/call list** are one-click presets that set status = No response, the new **Reachable** filter (`any` / has phone / no phone), and hide test rows. **Reminder language** (English/Español) controls the *guest-facing* copy only, independent of the admin UI language; the text comes from `admin.reminderMessage` in `src/i18n/dictionaries.ts` and its link is always built from `SITE.siteUrl` so a pasted message resolves to production. Copy a reminder per row (the **Reminder** link under the invite code, shown only for households without an RSVP) or for all selected rows at once (**Copy reminders**). **Export chase CSV** writes a working call/text sheet for the current filtered view (household, party limit, phone, reachable-by, city/state, RSVP link, EN and ES reminder text) — it is *not* a backup and is not importable; use **Export Master CSV (all)** for that.
...
- `src/routes/_authenticated/portal-ga-2026/notes.tsx` — **Notes & song requests** (admin-only, linked from the dashboard headcount panel next to the attendee report). Read-only over the same `listGuestsWithRsvps` rows the RSVPs tab loads; no schema change, no new server function. Lists every household whose RSVP has a non-empty `message` and/or `song_request`, newest first, with a search box (household/note/song), summary counts, a Messages card list, a Song requests table, "Copy song list" (plain text, one `song — household` per line, for the DJ), CSV export via `src/lib/csv.ts`, and the same print/Save-as-PDF styling as the attendee report. Test households (`ZZT…`) excluded by default, toggleable. Verified 2026-09-07 against the DB: 11 notes, 13 song requests among real households.
