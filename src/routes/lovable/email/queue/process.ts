...
// The JSONB payload shape enqueue_email() callers build (webhook.ts,
// transactional/send.ts) — not a DB-generated type since it's an
// application-level convention layered on top of the generic `message
// JSONB` column, not a real table shape. run_id/idempotency_key/
// unsubscribe_token are genuinely optional — only some callers set them
// (e.g. auth emails have no unsubscribe_token, transactional emails have
// no run_id) — everything else is set by every enqueue call site.
...
async function moveToDlq(
  ...
  const payload = msg.message as unknown as EmailQueuePayload;
        ...
        for (const queue of ["auth_emails", "transactional_emails"]) {
          ...
          const messageIds = Array.from(
            new Set(
              messages
                .map((msg) => {
                  const messageId = (msg.message as unknown as EmailQueuePayload | null)
                    ?.message_id;
          ...
          for (let i = 0; i < messages.length; i++) {
            ...
            // Drop expired messages (TTL exceeded). read_email_batch's
            // RETURNS TABLE only ever selects msg_id/read_ct/message (see
            // the migration) — there's no pgmq enqueued_at column to fall
            // back to, so queued_at must come from the payload itself,
            // which every enqueue_email() caller sets.
