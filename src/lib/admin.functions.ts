...
export const bulkSetPhotoStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: boolean; count: number }> => {
    const sb = await ensureAdmin(context.userId);
    const { error } = await sb.from("guest_photos").update({
      status: data.status,
      reviewed_by: context.userId,
      reviewed_at: new Date().toISOString(),
    }).in("id", data.ids);
...
export const deletePhoto = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: boolean }> => {
    ...
    const { data: row } = await sb
      .from("guest_photos").select("storage_path").eq("id", data.id).maybeSingle();
    if (row?.storage_path) {
...
export const bulkDeletePhotos = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: boolean; count: number }> => {
    const sb = await ensureAdmin(context.userId);
    const { data: rows } = await sb
      .from("guest_photos").select("storage_path").in("id", data.ids);
...
export const getPhotoCounts = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }): Promise<{ pending: number; approved: number; rejected: number }> => {
    const sb = await ensureAdmin(context.userId);
...
export const getRecentActivity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<{
    ...
    const [r24, r7, pPending, p7] = await Promise.all([
      ...
      sb.from("guest_photos").select("id", { count: "exact", head: true }).eq("status", "pending"),
      sb.from("guest_photos").select("id", { count: "exact", head: true }).gte("created_at", iso7d),
    ]);
    return {
